...
//handle print with dynamic configuration
ipcMain.handle("printComponent", async (event, printData) => {
  try {
    const { htmlContent, printerConfig } = printData;
    
    if (!htmlContent) {
      event.sender.send("print-result", {
        status: "error",
        message: "No HTML content provided for printing"
      });
      return { status: "error", message: "No HTML content provided" };
    }

    // Build print options from printer config or use defaults
